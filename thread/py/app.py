"""Flask App - Python implementation of thread/app.ts"""
...
<current_turn>User: {user_message.get('content')}\nAssistant: {assistant_response.get('content')}</current_turn>"""
    ...
    response = await openai_service.completion([summarization_prompt, {"role": "user", "content": "Please create/update our conversation summary."}], "gpt-4o-mini", False)
...
def create_system_prompt(summarization: str) -> dict:
    return {
        ...
        {'Here is a summary of the conversation so far: <conversation_summary>' + summarization + '</conversation_summary>' if summarization else ''}
        Let's chat!"""
    }
